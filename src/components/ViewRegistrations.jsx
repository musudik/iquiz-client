      ...
      try {
        ...
        if (data) {
          Object.entries(data).forEach(([id, registration]) => {
            if (registration.userDetails) {
              viewRegistrations.push({
                id,
                ...registration.userDetails,
                score: registration.userDetails.score || 0,
                answersCount: registration.answers?.length || 0
              });
            }
          });
        }
        
        setRegistrations(viewRegistrations);
        setError(null);
      } catch (err) {
          ...
          <TableBody>
            {paginatedRegistrations.map((registration) => (
              <TableRow key={registration.id}>
                ...
                <TableCell>{registration.score !== undefined ? `${registration.score}%` : 'Not started'}</TableCell>
                <TableCell>
